...
public class ClassMetamodelFactory implements ClassMetaConsts {
	/**
	 * table mapping primitive type names to corresponding class objects. This
	 * code was created by Carlos de Wolf on a fix of EJBTHREE-440. By
	 * coincidence I needed to parse primitive values, and instead of
	 * reinventing the wheel I got the same code.
	 */
	...
	/**
	 * We are caching the getClassLoader operation, to avoid locks on the
	 * WeakHashMap
	 */
	 ...
	 * >> (The current implementation is using FastHashMap instead of
	 * ConcurrentHashMap) *
	 */
	...
	public static ClassMetaData getClassMetaData(String clazzName, ClassResolver resolver, ClassLoader loader, boolean checkSerializable) throws IOException {
		try {
			...
			if (classMetadata.getClazz() == null) {
				...
				// The only possibility for that would be crossed classLoader
				// operations.
				// I tried to replicate this scenario on a testcase and it was
				// not possible.
			...
			// It looks dummy but doing Class.isProxy is a very expensive
			// operation, so we just do it on a cached basis
	...
	public static ClassMetaData getClassMetaData(Class clazz, boolean checkSerializable) throws IOException {
		...
		if (classMetadata.getClazz() == null) {
			...
			// The only possibility for that would be crossed classLoader
			// operations.
			// I tried to replicate this scenario on a testcase and it was not
			// possible.
			// So, I decided to have this verification here to be safe
		...
		// It looks dummy but doing Class.isProxy is a very expensive operation,
		// so we just do it on a cached basis
		if (classMetadata.isProxy()) {
			return proxyMetaData;
		} else {
			return classMetadata;
	...
	public static boolean isImmutable(Class clazz) {
		return (clazz == Character.class || clazz == String.class || clazz == Long.class || clazz == Byte.class || clazz == Double.class || clazz == Float.class || clazz == Integer.class
				|| clazz == Short.class || clazz == Boolean.class || clazz.isPrimitive());
	}
