...
public class FieldsContainer {
	...
	/**
	 * both {@link org.jboss.serial.persister.RegularObjectPersister) and
	 * writeMyself need to produce the same binary compatible output while it's
	 * not required by RegularObjectPersister to create an intermediate HashMap
	 * to read its fields. Becuase of that we have opted in keep static methods
	 * on FieldsContainer that will expose low level persistent operations
	 */
	...
	/**
	 * both {@link org.jboss.serial.persister.RegularObjectPersister) and
	 * readMyself need to produce the same binary compatible output while it's
	 * not required by RegularObjectPersister to create an intermediate HashMap
	 * to read its fields. Becuase of that we have opted in keep static methods
	 * on FieldsContainer that will expose low level persistent operations
	 */
