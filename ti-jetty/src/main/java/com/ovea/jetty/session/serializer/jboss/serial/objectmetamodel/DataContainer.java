...
public class DataContainer extends DataExport implements DataContainerConstants, Externalizable {
	/**
	 * These are the bytes used during streaming of Datacontainer. They are used
	 * to control and they are mixed within content. So, they should be read in
	...
	/**
	 * I used the transient tag as a documentation feature. this contains the
	 * root objectCache being used. It would be possible to use ThreadLocals for
	 * this. But I didn't want to take the risk of having problems when the
	 * application was running in ThreadPools generating leaks.
	 */
	...
	public DataContainer(ClassLoader loader, ObjectSubstitutionInterface substitution, boolean checkSerializable, StringUtilBuffer buffer, ClassDescriptorStrategy classDescriptorStrategy,
	...
	public DataContainer(ClassLoader loader, ObjectSubstitutionInterface substitution, SafeCloningRepository safeToReuse, boolean checkSerializable, StringUtilBuffer buffer,
			ClassDescriptorStrategy classDescriptorStrategy, ObjectDescriptorStrategy objectDescriptorStrategy) {
		this();
	...
	class DataContainerOutput implements ObjectsCache.JBossSeralizationOutputInterface {
		...
		/**
		 * outByte is used to hold writeByte operations until we start doing a
		 * different operation
		 */
	...
	class DataContainerInput implements ObjectsCache.JBossSeralizationInputInterface {
		...
		/*
		 * void testNextBytes() throws IOException { if(byteStreamInput==null) {
		 * moveNext(); }
		 * 
		 * if (this.byteStreamInput==null) { throw new
		 * SerializationException("Excepted a byteBuffer in the sequence"); } }
		 */
	...
	public void saveData(DataOutput output) throws IOException {
		// This only flushes internal buffers from sub models, it doesn't flush
		// to the external Streams.
		this.flush();
	 ...
	 * 
	 * @see
	 * org.jboss.serial.objectmetamodel.DataExport#writeMyself(java.io.DataOutput
	 * )
	 */
	public void writeMyself(DataOutput output) throws IOException {
		...
		while (iter.hasNext()) {
			...
			if (obj == null) {
				...
				// Although this was not supposed to happen as we will use array
				// of bytes, we still want to keep it just in
				// case
	...
	/**
	 * This is probably never used as we always use byteArrays for bytes, due to
	 * our implementation in the DataOutput.
	 *
	 * @param output
	 * @param obj
	 */
	 ...
	 * 
	 * @see
	 * org.jboss.serial.objectmetamodel.DataExport#readMyself(java.io.DataInput)
	 */
