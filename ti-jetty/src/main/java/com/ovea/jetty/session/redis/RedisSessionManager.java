...
public final class RedisSessionManager extends SessionManagerSkeleton<RedisSessionManager.RedisSession> {
	...
	private RedisSession loadFromStore(final String clusterId, final RedisSession current) {
		List<String> redisData = jedisExecutor.execute(new RedisCallback<List<String>>() {
			@Override
	...
	protected void storeSession(final RedisSession session) {
		if (!session.redisMap.isEmpty()) {
			final Map<String, String> toStore = session.redisMap.containsKey("attributes") ? session.redisMap : new TreeMap<String, String>(session.redisMap);
	...
	final class RedisSession extends SessionManagerSkeleton.SessionSkeleton {
		...
		protected void complete() {
			super.complete();
			if (!redisMap.isEmpty()
					&& (redisMap.size() != 3 || !redisMap.containsKey("lastAccessed") || !redisMap.containsKey("accessed") || !redisMap.containsKey("expiryTime") || getAccessed() - lastSaved >= saveIntervalSec * 1000)) {
				try {
					willPassivate();
					storeSession(this);
					didActivate();
